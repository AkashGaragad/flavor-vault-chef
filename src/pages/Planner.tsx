...
import { getPlanner, getRecipes, savePlanner, constants } from "@/store/recipeStore";
...
const Planner = () => {
  ...
  const onDrop = (day: string, meal: string, e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (!id) return;
    setPlanner((p) => ({ ...p, [day]: { ...p[day], [meal]: { recipeId: id, servings: 2 } } }));
  };
  ...
  return (
    <main className="container mx-auto py-8">
      <Helmet>
        ...
        <meta name="description" content="Plan your week with drag-and-drop scheduling for breakfast, lunch, and dinner." />
          ...
          <CardContent className="p-4 space-y-3 max-h-[70vh] overflow-auto">
            {recipes.map((r) => (
              ...
              >
                ...
                <div className="text-xs text-muted-foreground">{r.timeMinutes} min • {r.difficulty}</div>
              </div>
            ))}
          </CardContent>
        </Card>
                      ...
                      {recipe ? (
                        <>
                          ...
                          <div className="flex justify-between items-center text-xs text-muted-foreground">
                            ...
                            <Button size="sm" variant="outline" onClick={() => onClear(day, meal)}>Clear</Button>
                          </div>
                        </>
                      ) : (
                        <div className="h-full w-full text-sm text-muted-foreground flex items-center justify-center">Drop recipe here</div>
                      )}
                    </CardContent>
                  </Card>
                );
